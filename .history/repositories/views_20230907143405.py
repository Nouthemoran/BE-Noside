...
# Endpoint untuk menghasilkan token akses
# Ini akan otomatis menggunakan CustomTokenObtainPairView
# yang memerlukan otentikasi pengguna
@api_view(['POST'])
def obtain_token(request):
    return CustomTokenObtainPairView.as_view()(request)

# Endpoint ShowAll yang memerlukan otentikasi pengguna
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def ShowAll(request):
...
Dalam contoh di atas, kami menggunakan CustomTokenObtainPairView dengan IsAuthenticated permission untuk menghasilkan token akses. Kemudian, kami menghasilkan endpoint /api/token/ yang memerlukan otentikasi pengguna untuk mendapatkan token akses. Endpoint /api/repositories/list/ juga memerlukan otentikasi pengguna dengan menggunakan decorator @permission_classes([IsAuthenticated]).

Pastikan juga Anda telah mengonfigurasi URL Anda dengan benar untuk menggunakan endpoint-endpoint ini.
